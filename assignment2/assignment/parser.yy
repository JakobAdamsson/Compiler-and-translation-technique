...
%token <std::string> ELSE WHILE PRINT EQUALSIGN PUBLIC STATIC VOID MAIN STRING CLASS TOKEN_DOLLAR TOKEN_CARET
...
Expression: Term 
            ...
            | Expression DOT ID LPAREN RPAREN
            {
              ...
              $$->children.push_back(new Node("Arguments", "", yylineno));
            }
...
Statement: LBRACE LRStatement RBRACE
            {
              $$ = $2;
            }
            | RBRACE LBRACE
            ...
            |ID LBRACKET Expression RBRACKET EQUALSIGN Expression SEMICOLON
            {
...
MethodDeclaration: PUBLIC Type ID LPAREN LRParamater RPAREN LBRACE LRVarOrStatementDec RETURN Expression SEMICOLON RBRACE
            ...
            | PUBLIC Type ID LPAREN RPAREN LBRACE LRVarOrStatementDec RETURN Expression SEMICOLON RBRACE
            {
              std::string dtype = $2->type;
              // CHANGE TO $2 HERE IF IT BUGS!
              $$ = new Node("MethodDeclaration", $3, yylineno, dtype);
            ...
            |PUBLIC Type ID LPAREN RPAREN LBRACE RETURN Expression SEMICOLON RBRACE
            {
              std::string dtype = $2->type;
              // CHANGE TO $2 HERE IF IT BUGS!
              $$ = new Node("MethodDeclaration", $3, yylineno, dtype);
            ...
            | PUBLIC Type ID LPAREN LRParamater RPAREN LBRACE RETURN Expression SEMICOLON RBRACE
            {
              std::string dtype = $2->type;
              // CHANGE TO $2 HERE IF IT BUGS!
              $$ = new Node("MethodDeclaration", $3, yylineno, dtype);
...
ClassDeclaration:CLASS ID LBRACE LRVarDec LRMethodDec RBRACE
            ...
            | CLASS ID LBRACE LRVarDec RBRACE
            {
              $$ = new Node("ClassDeclaration", $2, yylineno);
              $$->children.push_back(new Node("Identifier", $2, yylineno));
              $$->children.push_back($4);
            ...
            | CLASS ID LBRACE LRMethodDec RBRACE
            {
              $$ = new Node("ClassDeclaration", $2, yylineno);
              $$->children.push_back(new Node("Identifier", $2, yylineno));
              $$->children.push_back($4);
