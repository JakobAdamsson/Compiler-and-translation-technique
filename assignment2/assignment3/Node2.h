...
public:
	...
	void create_symboltable(SymbolTable *symboltable)
	{
		...
		for (auto i = children.begin(); i != children.end(); i++)
		{
			...
			{
				std::cout << "VAD STÅR DET HÄR?" << symboltable->lookup(this->value).id << std::endl;
				Record test = symboltable->lookup(this->value);
				std::cout << "heeeeeeeeeeeeeeeeeeeeeeeeeeej" << test.id << test.type << std::endl;
	...
	void getMethodData(Node *i, Method *new_method)
	{
		...
		{
			// std::cout << "type: " << (*itr)->type << " value: " << (*itr)->value << std::endl;
			...
			else if ((*itr)->value == this->ifstate)
			{
				...
				new_ifstatement.id = (*itr)->value;
				new_ifstatement.type = (*itr)->type;
				new_method->addStatement((*itr)->value, new_ifstatement);
			}
