        ...
        for (Node *child : node->children)
        {
            if (child->type == "If statement")
            {
                BBlock
                    ...
                    Node *condition_child = child->children[0];
                handle_condition(symboltable, condition_child, current_block);

                Node *true_child = child->children[1];
                BBlock *true_block = handle_true_block(symboltable, true_child, current_block);
                old_block->trueExit = true_block;

                Node *else_child = child->children[2]->type == "EMPTY" ? nullptr : child->children[2];
                BBlock *false_block = handle_else(symboltable, else_child, current_block);

                BBlock *join_block = new BBlock();
                join_block->name = "JoinBlock" + std::to_string(symboltable->num_blocks++);
                if ((else_child != nullptr))
                {
                    old_block->falseExit = false_block;
